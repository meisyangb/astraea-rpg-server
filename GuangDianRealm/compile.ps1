...
$ProjectDir = "e:\RPG\plugins\GuangDianRealm"
...
$Libs = @(
    ...
    "e:\RPG\plugins\GuangDianArmorStats\build\libs\GuangDianArmorStats.jar",
...
Write-Host "正在编译 GuangDianRealm 境界插件..." -ForegroundColor Cyan
...
$JarFile = "$ProjectDir\build\GuangDianRealm-1.0.0.jar"
& "$JAVA_HOME\bin\jar.exe" cvf $JarFile -C $OutputDir .

Write-Host "编译成功! JAR文件: $JarFile" -ForegroundColor Green