...
$env:PROJECT_DIR = "e:\RPG\plugins\RPGItems"
...
# 创建jar文件
Write-Host "创建jar文件..."
& "$env:JAVA_HOME\bin\jar.exe" cvf "$env:PROJECT_DIR\build\RPGItems-1.0.0.jar" -C "$env:PROJECT_DIR\build\classes" .

# 部署到艾尔丽雅服务器
Write-Host "部署到艾尔丽雅服务器..."
Copy-Item -Path "$env:PROJECT_DIR\build\RPGItems-1.0.0.jar" -Destination "e:\RPG\艾尔丽雅\plugins\RPGItems-1.0.0.jar" -Force
