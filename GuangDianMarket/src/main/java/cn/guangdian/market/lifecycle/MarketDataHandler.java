...
public class MarketDataHandler extends AbstractPlayerDataHandler {
    ...
    protected void onPlayerLoad(Player player) {
        ...
        if (returns != null && !returns.isEmpty()) {
            ...
            player.sendMessage(plugin.getMiniMessageService().parseWithPlaceholders(
                plugin.getConfig().getString("messages.offline-returns", "<yellow>你有 <count> 个过期物品已返还!"),
                "count", String.valueOf(returns.size())));
        }
    }
