...
public class ConfigManager {
    ...
    public void load() {
        ...
        prefix = config.getString("messages.prefix", "<gold>[图鉴] <reset>");
    }
    ...
    public Component getMessageWithPlaceholders(String key, String... keyValues) {
        MiniMessageService mm = plugin.getMiniMessage();
        if (mm == null) {
            return Component.text(getMessage(key));
        }
        String template = getMessage(key);
        return mm.parseUnified(template, keyValues);
    }
    
    public Component getColoredMessage(String key) {
        MiniMessageService mm = plugin.getMiniMessage();
        if (mm == null) {
            return Component.text(getMessage(key));
        }
        return mm.colorize(getMessage(key));
    }
}
