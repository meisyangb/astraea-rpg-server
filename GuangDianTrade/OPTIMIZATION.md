...
settings:
  prefix: "&6[&e交易&6] "
messages:
  no-permission: "&c你没有权限使用交易功能!"
```

**更新后:**
```yaml
settings:
  prefix: "<gold>[<yellow>交易<gold>] "
messages:
  no-permission: "<red>你没有权限使用交易功能!"
```
...
- ✅ `SyncScheduler` 任务调度器（通过 RPGCore.getScheduler()）
- ✅ `TradeServiceAdapter` 服务适配器
