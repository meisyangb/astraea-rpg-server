...
public class DamageManager {
    ...
    public void handlePlayerAttack(EntityDamageByEntityEvent event) {
        ...
        // 修复: 检查玩家是否有 RPG 攻击属性，而不是检查武器
        // 如果玩家有 RPG 攻击属性（来自防具、配饰等），就使用 RPG 伤害计算
        // 如果玩家没有 RPG 攻击属性，就使用原版伤害
        boolean hasRpgAttack = hasRpgAttackAttribute(attackerStats, isPVP);
        boolean isVanillaAttack = !isSkillDamage && !hasRpgAttack;
        ...
        // 修复: 原版武器攻击也应该应用玩家的 RPG 属性（来自防具、配饰等）
        // 移除原来的 isVanillaAttack 提前返回逻辑
        // 玩家的 RPG 攻击力应该始终生效，不管手持什么武器

        // 如果玩家没有 RPG 攻击属性，使用原版伤害
        if (isVanillaAttack) {
     ...
     * @param stats 玩家属性
     * @param isPVP 是否是 PVP
     * @return 如果玩家有任何攻击属性返回 true
     */
    private boolean hasRpgAttackAttribute(PlayerStats stats, boolean isPVP) {
        if (stats == null) {
            return false;
        }
        
        // 检查普通攻击力
        if (stats.getMinAttack() > 0 || stats.getMaxAttack() > 0) {
            return true;
        }
        
        // 如果是 PVP，还要检查 PVP 攻击力
        if (isPVP) {
            if (stats.getPvpMinAttack() > 0 || stats.getPvpMaxAttack() > 0) {
                return true;
            }
        }
