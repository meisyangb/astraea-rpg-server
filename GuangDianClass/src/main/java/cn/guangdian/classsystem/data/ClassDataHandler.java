...
public class ClassDataHandler extends AbstractPlayerDataHandler {
    ...
    private PlayerClassData loadPlayerData(UUID playerId) {
        ...
        int initialPoints = plugin.getConfig().getInt("attributes.initial-points", 50);
        newData.setAttributePoints(initialPoints);
        return newData;
    }
    
    public PlayerClassData getPlayerData(UUID playerId) {
        return dataCache.computeIfAbsent(playerId, id -> loadPlayerData(id));
    }
    
    public void savePlayerData(UUID playerId, PlayerClassData data) {
        if (data == null) return;
        
        if (rpgCore != null) {
            rpgCore.getScheduler().runAsync(() -> {
                savePlayerDataSync(playerId, data);
            });
        } else {
            savePlayerDataSync(playerId, data);
        }
    ...
    private void savePlayerDataSync(UUID playerId, PlayerClassData data) {
        String path = "players." + playerId.toString();
