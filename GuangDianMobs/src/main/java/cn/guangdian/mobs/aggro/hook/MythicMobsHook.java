package cn.guangdian.mobs.aggro.hook;
...
public class MythicMobsHook {
    ...
    public void init() {
        ...
        if (plugin == null || !plugin.isEnabled()) {
            Bukkit.getLogger().info("[GuangDianMobs] MythicMobs 未安装，跳过仇恨系统集成");
            return;
        }
        ...
        try {
            ...
            enabled = true;
            Bukkit.getLogger().info("[GuangDianMobs] MythicMobs 仇恨集成已启用");
        } catch (Exception e) {
            Bukkit.getLogger().warning("[GuangDianMobs] MythicMobs 仇恨集成失败: " + e.getMessage());
            enabled = false;
        }
    }
