...
public class FakePlayerCommand implements CommandExecutor, TabCompleter {
    ...
    public FakePlayerCommand(GuangDianFakePlayer plugin) {
        this.plugin = plugin;
    }
    ...
    private void handleReload(CommandSender sender) {
        if (!sender.hasPermission("gdfakeplayer.admin")) {
    ...
    private void handleStatus(CommandSender sender) {
        ...
        sender.sendMessage(ChatColor.AQUA + "=== GuangDianFakePlayer 状态 ===");
        sender.sendMessage(ChatColor.YELLOW + "模式: " + config.getMode());
        sender.sendMessage(ChatColor.YELLOW + "真实在线: " + realOnline);
        sender.sendMessage(ChatColor.YELLOW + "虚拟人数: " + virtualCount);
        sender.sendMessage(ChatColor.YELLOW + "伪装在线: " + fakeOnline + " (真实" + realOnline + " + 虚拟" + virtualCount + ")");
        sender.sendMessage(ChatColor.YELLOW + "伪装最大: " + config.getFakeMaxPlayers());
        sender.sendMessage(ChatColor.YELLOW + "PlaceholderAPI: " + (config.isPlaceholderapiEnabled() ? "启用" : "禁用"));
        sender.sendMessage(ChatColor.YELLOW + "占位符前缀: %" + config.getPlaceholderPrefix() + "_online%");
    }
    ...
    private void handleSet(CommandSender sender, String[] args) {
        ...
        if (args.length < 3) {
            sender.sendMessage(ChatColor.RED + "用法: /gdfakeplayer set <参数> <值>");
            sender.sendMessage(ChatColor.YELLOW + "可用参数: fixed-online, min-online, max-online, multiplier, fake-max-players");
            return;
        }
    ...
    private void handleMode(CommandSender sender, String[] args) {
        ...
        if (args.length < 2) {
            sender.sendMessage(ChatColor.RED + "用法: /gdfakeplayer mode <模式>");
    ...
    private void sendHelp(CommandSender sender) {
        sender.sendMessage(ChatColor.AQUA + "=== GuangDianFakePlayer 帮助 ===");
        sender.sendMessage(ChatColor.YELLOW + "/gdfakeplayer reload - 重新加载配置");
        sender.sendMessage(ChatColor.YELLOW + "/gdfakeplayer status - 查看当前状态");
        sender.sendMessage(ChatColor.YELLOW + "/gdfakeplayer set <参数> <值> - 设置参数");
        sender.sendMessage(ChatColor.YELLOW + "/gdfakeplayer mode <模式> - 切换模式");
    }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length == 1) {
            ...
            if (args[0].equalsIgnoreCase("set")) {
                completions.addAll(Arrays.asList("fixed-online", "min-online", "max-online", "multiplier", "fake-max-players"));
