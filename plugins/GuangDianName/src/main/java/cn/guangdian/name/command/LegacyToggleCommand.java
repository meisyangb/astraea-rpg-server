...
/**
 * 切换命令处理器 - 传统 Bukkit 实现 (降级方案)
 * 当 RPGCore CommandFramework 不可用时使用
 */
public class LegacyToggleCommand implements CommandExecutor, TabCompleter {

    private final GuangDianName plugin;
    private final NameDisplayManager displayManager;
    private final MiniMessageService miniMessage;

    public LegacyToggleCommand(GuangDianName plugin, NameDisplayManager displayManager) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        switch (subCommand) {
            ...
            case "help":
                ...
                break;
            default:
                player.sendMessage(miniMessage.colorize("<red>未知的子命令! 使用 /gdnametoggle help 查看帮助"));
                break;
        }
