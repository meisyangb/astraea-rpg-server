...
public abstract class BaseCommand {
    ...
    public void showHelp(@NotNull CommandSender sender) {
        ...
        if (info == null) {
            msg.sendMessage(sender, "&c命令配置错误!");
            return;
        }

        msg.sendMessage(sender, "&6========== " + info.description() + " ==========");
        ...
        for (Method method : getSubCommandMethods()) {
            ...
            String commandText = "&e/" + info.name() + " " + subCmd.name();
            String descriptionText = desc != null ? " &7- " + desc.value() : "";

            msg.sendMessage(sender, commandText + descriptionText);
        }

        msg.sendMessage(sender, "&6==================================");
    }
