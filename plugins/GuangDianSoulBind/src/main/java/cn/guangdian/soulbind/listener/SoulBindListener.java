...
public class SoulBindListener implements Listener {
    ...
    public void onEntityPickupItem(EntityPickupItemEvent event) {
        if (!config.isAutoBindOnPickup()) return;
        if (!(event.getEntity() instanceof Player player)) return;

        ItemStack item = event.getItem().getItemStack();

        if (config.isMythicMobsOnly()) {
            if (!mythicMobsHook.isMythicItem(item)) return;

            String itemId = mythicMobsHook.getMythicItemId(item);
            if (!config.shouldBindItem(itemId)) return;
        }

        if (service.isBound(item)) return;

        if (service.bindItem(item, player)) {
            sendMessage(player, "bind-success");
        }
    }
