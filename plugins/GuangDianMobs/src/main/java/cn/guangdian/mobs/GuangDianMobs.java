...
public class GuangDianMobs extends AbstractRPGPlugin {
    ...
    private AggroService aggroService;
    ...
    protected void onPluginEnable() {
        ...
        // 获取仇恨服务（可选依赖）
        try {
            aggroService = rpgCore.getServiceRegistry().getService(AggroService.class);
            if (aggroService != null) {
                getLogger().info("已集成仇恨系统");
            }
        } catch (Exception e) {
            getLogger().info("仇恨系统不可用，跳过集成");
        }
    ...
    public AggroService getAggroService() { return aggroService; }
}
