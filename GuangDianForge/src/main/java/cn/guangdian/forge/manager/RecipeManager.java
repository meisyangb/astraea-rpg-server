...
public class RecipeManager {
    ...
    private ForgeRecipe parseRecipe(String id, ConfigurationSection rs) {
        ...
        long expReward = rs.getLong("exp-reward", 50); // 默认经验值，可在每个图纸中独立配置
        
        // 解析材料 - 格式 "rpg:物品ID" 或 "rpgitems:物品ID"
        ...
        if (resultSection != null) {
            ...
            if (resultRPGItem == null || resultRPGItem.isEmpty()) {
                // 尝试旧的配置格式
                resultRPGItem = resultSection.getString("item");
            }
        }
        
        if (resultRPGItem == null || resultRPGItem.isEmpty()) {
            throw new IllegalArgumentException("图纸 " + id + " 必须配置 result.rpg-item");
        }
        ...
        return new ForgeRecipe(id, name, requiredLevel, ingredients, resultRPGItem,
                              baseRate, expReward, blueprintDisplay, blueprintLore, isBlueprintBook);
    }
